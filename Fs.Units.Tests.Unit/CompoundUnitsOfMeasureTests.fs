...
let areaTests config =
    ...
    let squareInchConversionTests =
        [ "From square inch to square feet and back",
          ...
          (fun x -> SquareInch.create x |> SquareInch.toSquareMeters |> SquareMeter.toSquareInches) ]
    ...
    let squareMeterConversionTests =
        [ "From square meter to square inches and back",
          (fun x -> SquareMeter.create x |> SquareMeter.toSquareInches |> SquareInch.toSquareMeters)
...
let pressureTests config =
    ...
    let pascalConversionTests =
        [ "From pascal to pounds per square inch",
          (fun x -> Pascal.create x |> Pascal.toPoundsPerSquareInch |> PoundPerSquareInch.toPascals) ]
        |> List.map (fun (x, y) -> testConversionRoundingError config Accuracy.high x y)
...
let electricChargeTests config =
    ...
    let ampereHourConversionTests =
        [ "From ampere hour to coloumbs and back",
          ...
          (fun x -> AmpereHour.create x |> AmpereHour.toAmpereSeconds |> AmpereSecond.toAmpereHours) ]
        |> List.map (fun (x, y) -> testConversionRoundingError config Accuracy.high x y)
