...
module Angle =
    ...
    module Degree =
        ...
        let toGradians (x: float<deg>) =
            x * (200.0 / 180.0) * 1.0<grad> / 1.0<deg>
    ...
    module Radian =
        ...
        let toRevolutions (x: float<rad>) =
            x * 1.0<rev> / (2.0 * Math.PI * 1.0<rad>)

        let toArcMinutes (x: float<rad>) =
            x * 60.0<arcmin> * 180.0 / Math.PI / 1.0<rad>

        let toArcSeconds (x: float<rad>) =
            x * 3600.0<arcs> * 180.0 / Math.PI / 1.0<rad>
    ...
    module Gradian =
        ...
        let toDegrees (x: float<grad>) =
            x * 1.0<deg> / (200.0 / 180.0) / 1.0<grad>
    ...
    module ArcMinute =
        ...
        let toRadians (x: float<arcmin>) =
            x * Math.PI * 1.0<rad> / (180.0 * 60.0) / 1.0<arcmin>
    ...
    module ArcSecond =
        ...
        let toRadians (x: float<arcs>) =
            x * Math.PI * 1.0<rad> / (180.0 * 3600.0) / 1.0<arcs>
